...
Para obtener una muestra representativa para el tamaño de tareas y tiempo de ejecución de las mismas, en este trabajo se hace uso de \textit{Odoo}, que es un sistema completo de gestión empresarial, de código abierto y sin costes de licencias que cubre las necesidades de las áreas de: Contabilidad y Finanzas, Ventas, RRHH, Compras, Proyectos, entre otras. (\citeauthor{odooWiki}, \citeyear{odooWiki}).
...
\textit{Odoo} nos ofrece una plataforma Web (Figura \ref{fig:loginOdoo}), en donde podemos administrar nuestras aplicaciones, además de permitir instalar aún más módulos que se pueden encontrar de manera gratuita o con algunas licencias por parte de sus desarrolladores (Figura \ref{fig:OdooApps}).
...
Una vez realizadas las operaciones se procedió a revisar el tiempo de respuesta del servidor así como el tamaño de las tareas para poder adecuar los parámetros en nuestro entorno de simulación, esto a través de las herramientas para desarrolladores del navegador que nos permitieron listar las tareas y filtrarlas por tiempo de respuesta (Figura \ref{fig:RequestOdooByTime}) y por tamaño de la petición (Figura \ref{fig:RequestOdooBySize}).
